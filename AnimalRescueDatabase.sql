create table location
	(
		...
		location_type enum("spca", "rescue_org", "shelter", "adopter") not null,
...
create table shelter
	(
		...
		max_dogs numeric(4, 0) not null default 0,
		max_cats numeric(4, 0) not null default 0,
		max_rabbits numeric(4, 0) not null default 0,
		max_rodents numeric(4, 0) not null default 0,
		foreign key (location_id) references location (location_id),
		primary key (ID)
	);
...
create table animal
	(
		ID varchar(5),
		animal_type enum("dog", "cat", "rabbit", "rodent") not null,
		location_id varchar(5) not null,
		foreign key (location_id) references location (location_id),
		primary key (ID)
	);
